# Measuring motion using the IMU

Created: August 20, 2025 9:37 PM
Owner: Geoff McIntyre

!!! warning "Are you skipping ahead?"

    Make sure you have already installed the custom tinyCore board library and setup your Arduino IDE using the [Get Started tutorial](arduino-ide.md)!

Okay, but first:
# What the hell is an "IMU"?

An IMU stands for "Inertial Measurement Unit", but basically, it's a motion tracker! The tinyCore's IMU has **6 "Degrees of Freedom"** (DOF), meaning we are measuring 6 things: Acceleration in the X, Y, and Z axis, and Rotation (gyroscopic motion to be precise) in the X, Y, and Zed axis. (Sorry, got a little British there.) 

This is also known as Pitch, Yaw, and Roll for all you aerospace nerds.

![6DOF.png](imu/6DOF.png)

The IMU on the tinyCore is specifically the LSM6DSOX, manufactured by STMicroelectronics. Within the datasheet, it describes it's orientation:

![image.png](imu/image11.png)

And here is how that chip is oriented on the board.

![image.png](imu/image12.png)

Now that you know where it is, let's walk through how to use it with the Arduino IDE. At this point, you should have [**already installed the custom board library**](arduino-ide.md) and tested flashing code with an example program (e.g. Blink).

### 1. Install the libraries

The LSM6DSOX has a wonderful Arduino library made by Adafruit: [**Adafruit_LSM6DS**](https://github.com/adafruit/Adafruit_LSM6DS/tree/master)
...
If you want to go further, you can learn how to improve our measurements via thresholding and Kalman filters in our IMU Deep Dive.

Nice job, now let's move on to your first project!
